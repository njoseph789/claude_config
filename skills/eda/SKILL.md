...
**Every `.ipynb` notebook generated by this skill MUST begin with the following path setup as its first code cell** (preceded by a markdown cell explaining what it does):

**Markdown cell (first cell):**
```markdown
## Setup

Configure root path by walking up from current working directory until `data/` directory is found. Builds `DATA_DIR` pointing to `data/full/`.
```

**Code cell (second cell — always first code cell):**
```python
...
for _ in range(4):
    if (ROOT / "data").is_dir():
        break
    ROOT = ROOT.parent
DATA_DIR = ROOT / "data" / "full"
```

This pattern:
- Is cross-platform (no hardcoded absolute paths)
- Works regardless of where the notebook is opened from
- Walks up at most 4 levels to find the `data/` directory
- All subsequent file loads must use `DATA_DIR / "filename"` — never `pd.read_csv("/absolute/path/...")`

Any data loading cells that follow must reference `DATA_DIR` (or `ROOT`) built above, not hardcoded paths.
